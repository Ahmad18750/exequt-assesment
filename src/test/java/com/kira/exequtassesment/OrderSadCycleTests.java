...
public class OrderSadCycleTests {
    ...
    @Test
    @Order(2)
    public void testOrderPaymentSuccessAfterFailure() {
        ...
        Boolean requestBody = false; // Simulate failed payment
        ...
        // restart the payment process to ensure it can be retried after failure
        paymentStartResponse = restTemplate.postForEntity("http://localhost:8080/orders/"+orderId+"/payment/start", null, String.class);
        assertThat(paymentStartResponse.getStatusCode().is2xxSuccessful()).isTrue();
        ...
        requestBody = true; // Simulate successful payment
        paymentWebhookResponse = restTemplate.postForEntity("http://localhost:8080/payments/webhook/"+paymentId, requestBody, String.class);
        assertThat(paymentWebhookResponse.getStatusCode().is2xxSuccessful()).isTrue();
